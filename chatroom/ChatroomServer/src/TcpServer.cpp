...
void TcpServer::Run() {
    ...
    Channel* channel = new Channel(
        m_lfd, FDEvent::ReadEvent, acceptConnection, nullptr, nullptr,
        this);  // TODO //只要服务器还在运行监听就不会停止，
...
int TcpServer::acceptConnection(void* arg) {
    ...
    // 将cfd放到TcpConnection处理 evLoop放到子线程
    // 建立连接在主线程，和客户端通信都在子线程中运行 匿名对象
    new TcpConnection(cfd, evLoop, server->m_redis, server->m_onlineUsersPtr_);
    return 0;
}